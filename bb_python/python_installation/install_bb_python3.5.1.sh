#!/usr/bin/env bash
#
# Script name: install_bb_python3.5.1.sh
#
# Description: Script to install python 3.5.1 (Recommended)
#
#
...
mv Python-${py_v} Python-${py_v}_empty
cd Python-${py_v}_empty
./configure --prefix=`pwd`/../../bb_python${py_v}_empty
make
make install

cd ../../bb_python${py_v}_empty/bin
...
./$py_r virtualenv `pwd`/../../bb_python-${py_v}
. ../../bb_python-${py_v}/bin/activate
...
if [ "$py_v" = "3.5.1" ] ; then
    for elem in `find . -name "*.py"` ; do python ../../bb_python3.5.1_empty/bin/2to3 -w $elem ; done
fi
python setup.py install

#Clear not-needed files
cd ../..
rm -rf bb_python-${py_v}/bin/get-pip python_installation/gradunwarp_FMRIB python_installation/Python-$py_v python_installation/Python-$py_v.tgz
