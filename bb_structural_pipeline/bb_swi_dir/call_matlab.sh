...
# Uses the environment variable FSL_SWI_MATLAB_MODE to control whether compiled, un-compiled or Octave versions are used
...
"$0 [-z <mode>] [-o <options>] [-p <path>] [-h <fix path>] 
	...
	-z <mode>:	This overrides the environment variable 
		FSL_SWI_MATLAB_MODE which would	otherwise control this.
	...
	-o <options>: Double quoted list of MATLAB/Octave command line
		options. Defaults to \${FSL_SWI_MLOPTS} or \${FSL_SWI_OCOPTS}
		respectively
	-p <path>: Path to MATLAB/Octave startup script/binary.
		Defaults to: \${FSL_SWI_MATLAB} or \${FSL_SWI_OCTAVE}
		respectively
	...
	-c <mcr path>: Path to MATLAB Compiler Runtime for the version
		of MATLAB Fix was compiled under. Defaults to \${FSL_SWI_MCRROOT}
	-b <compiled path>: Path to the folder containing the compiled MATLAB
		functions. Defaults to \${FSL_SWI_MLCDIR}
...
do
	...
	h)
		if [ -z "${SWI_PATH}" ]; then
			...
			echo "More than one SWI path passed." >&2
			usage
		fi
		;;
	w)
...
if [ -z "${fmm}" ]; then
	if [ -z "${FSL_SWI_MATLAB_MODE}" ]; then
		fmm=0
	else
		fmm=${FSL_SWI_MATLAB_MODE}
	fi
fi	
...
if [ -z "${MATLAB_BIN}" ]; then
	MATLAB_BIN=${FSL_SWI_MATLAB}
fi
if [ -z "${OCTAVE_BIN}" ]; then
	OCTAVE_BIN=${FSL_SWI_OCTAVE}
fi
if [ -z "${CSUBDIR}" ]; then
	CSUBDIR=${FSL_SWI_MLCDIR}
fi
if [ -z "${MCR}" ]; then
	MCR=${FSL_SWI_MCR}
fi
...
if [ -z "${DOPTS}" ]; then
	if [ "${fmm}" -eq 1 ]; then
		DOPTS=${FSL_SWI_MLOPTS}
	else
		DOPTS=${FSL_SWI_OCOPTS}
	fi
fi
...
then
	RUNTIME="${MATLAB_BIN}"
	EVAL_CMD=${FSL_SWI_MLEVAL}
	FILE_CMD=${FSL_SWI_MLFILE}
	if [ ! -x "${RUNTIME}" ]; then
echo "${RUNTIME} not found!" >&2
	...
	RUNTIME="${OCTAVE_BIN}"
	EVAL_CMD=${FSL_SWI_OCEVAL}
	FILE_CMD=${FSL_SWI_OCFILE}
	if [ ! -x "${RUNTIME}" ]; then
echo "${RUNTIME} not found!" >&2
		usage
	fi
...
if [ -z "${FSL_MATLAB_PATH}" ]; then
	FSL_MATLAB_PATH=${FSL_SWI_FSLMATLAB}
fi

ML_PATHS="addpath('${FSL_SWIDIR}'); addpath('${FSL_MATLAB_PATH}');"
