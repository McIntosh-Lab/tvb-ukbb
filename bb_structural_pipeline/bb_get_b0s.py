...
def main():    
    ...
    parser.add_argument('-n', dest='desiredNumber',type=int, default=[4], nargs=1,help='Desired number of B0s from file. (Default: 4)')
    parser.add_argument('-l', dest='B0limit',type=int, default=[100], nargs=1,help='Limit B0 value. (Default 50)')
    ...
    if argsa.desiredNumber[0] > len(indices):
        print(("There are only %i B0. It is not possible to have %i" % (len(indices), argsa.desiredNumber[0])))
        exit()
    ...
    output = check_output('$FSLDIR/bin/fslselectvols -i ' + argsa.inputFile[0] + ' -o ' + argsa.outputFile[0] + ' --vols=' + ','.join(str(i) for i in indices), shell=True)
