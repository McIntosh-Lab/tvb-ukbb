...
sys.path.insert(
    1, os.path.dirname(__file__) + "/../bb_python/python_installation/gradunwarp_FMRIB"
)
...
def argument_parse_gradunwarp():
    ...
    # required arguments
    p.add_argument(
        "infile", action="store", help="The input warped file (nifti or mgh)"
    )
    p.add_argument(
        "outfile",
        action="store",
    ...
    coef_grp.add_argument(
        "-g", "--gradfile", dest="gradfile", help="The .grad coefficient file"
    )
    coef_grp.add_argument(
        "-c", "--coeffile", dest="coeffile", help="The .coef coefficient file"
    )
    ...
    p.add_argument(
        "--numpoints", dest="numpoints", help="number of grid points in each direction"
    )
    p.add_argument(
        "--interp_order",
        dest="order",
